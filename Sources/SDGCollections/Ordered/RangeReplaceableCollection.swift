...
extension RangeReplaceableCollection {
  ...
  @inlinable internal mutating func appendAsCollection<S>(contentsOf newElements: S)
  ...
  @inlinable internal mutating func insertAsCollection<S>(
    contentsOf newElements: S,
    at i: Self.Index
  ) where S: Collection, S.Element == Self.Element {
  ...
  @inlinable internal mutating func replaceSubrangeAsCollection<C>(
  ...
  @inlinable internal mutating func replaceSubrangeAsCollection<R, C>(
    _ subrange: R,
    with newElements: C
  ) where R: RangeExpression, R.Bound == Self.Index, C: Collection, C.Element == Self.Element {
    replaceSubrange(subrange, with: newElements)
  }
  ...
  @inlinable internal func appendingAsCollection<C: Collection>(contentsOf newElements: C) -> Self
  where C.Element == Self.Element {
  ...
  @inlinable internal mutating func prependAsCollection<C: Collection>(contentsOf newElements: C)
  where C.Element == Self.Element {
  ...
  @inlinable internal func prependingAsCollection<C: Collection>(contentsOf newElements: C) -> Self
  where C.Element == Self.Element {
    ...
    -> Self
  {
    return nonmutatingVariant(
      of: { $0.fill(to: $1, with: $2, from: $3) },
      on: self,
      with: (count, element, direction)
    )
  }
  ...
  @inlinable public func applying(changes: CollectionDifference<Element>) -> Self? {
    if #available(macOS 10.15, iOS 13, watchOS 6, tvOS 13, *),
      ¬legacyMode
    {
      // @exempt(from: tests) #workaround(workspace version 0.27.1, CI doesn’t run 10.15 yet.)
