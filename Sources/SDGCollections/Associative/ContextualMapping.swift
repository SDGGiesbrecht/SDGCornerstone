...
Output : RangeReplaceableCollection {
        ...
        for (input, output) in rules {
            ...
            guard let first = input.first else {
                _preconditionFailure({ localization in
                    switch localization {
                    case .englishCanada:
                        return "Empty rule input: “\(String(describing: input))” → “\(String(describing: output))”"
                    }
                })
            }
    ...
    @inlinable public init(exhaustiveMapping: [Input: Output]) {
        self.init(mapping: exhaustiveMapping, fallbackAlgorithm: ContextualMapping.defaultFallbackAlgorithm)
    }
    ...
    /// - Parameters:
    ///     - input: The input.
    @inlinable public func map(input: Input) -> Output {
        var remainder = input
        var output = Output()
