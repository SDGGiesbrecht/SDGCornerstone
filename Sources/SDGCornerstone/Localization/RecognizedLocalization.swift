...
internal enum RecognizedLocalization : String, Localization {
    ...
    internal static let cases: [RecognizedLocalization] = [
    ...
    private var flag: StrictString {
        ...
        return StrictString(country.scalars.map({ UnicodeScalar($0.value + RecognizedLocalization.flagOffset)! }))
    }
    ...
    private var abbreviation: StrictString {
        ...
        case .조선말조선:
            return "조"
        default:
            return RecognizedLocalization.codeToAbbreviation[code.components(separatedBy: "\u{2D}").dropLast().joined(separator: "\u{2D}")]!
        }
    }
    ...
    internal init?(definedIcon: StrictString) {
        ...
        let country = RecognizedLocalization.code(for: flag)
        guard let language = RecognizedLocalization.abbreviationToCode[abbreviation] else {
            return nil
        }
    ...
    internal static let fallbackLocalization: RecognizedLocalization = .עברית־ישראל
}
