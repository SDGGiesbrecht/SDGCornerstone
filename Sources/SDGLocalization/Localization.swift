  ...
  internal init?(reasonableMatchFor code: String, skippingParents: Bool) {
      ...
      if let script = possibleScript {
        ...
        if let country = possibleCountry {
          ...
          for entry in scripts where entry.countries.contains(country) {
            if let result = Self(
              exactly: [language, entry.script, country].joined(separator: "\u{2D}")
            ) {
              self = result
              return
            }
          ...
          for entry in scripts {
            for country in entry.countries {
              if let result = Self(
                reasonableMatchFor: [language, entry.script].joined(separator: "\u{2D}"),
                skippingParents: true
              ) {
                self = result
                return
              }
              if let result = Self(
                reasonableMatchFor: [language, country].joined(separator: "\u{2D}"),
                skippingParents: true
              ) {
                self = result
                return
    ...
    if let members = ContentLocalization.macrolanguages[language] {
      for member in members {
        ...
        if let result = Self(
          reasonableMatchFor: alteredTags.joined(separator: "\u{2D}"),
          skippingParents: true
        ) {
          self = result
          return
