...
public func testWholeArithmeticConformance<T>(
  ...
  line: UInt = #line
) where T: WholeArithmetic & _ComparableIfNotInherited & _NumericIfNotInherited & _WholeArithmeticRandomness {
  ...
  testNumericAdditiveArithmeticConformance(
    ...
    addend: 9,
    sum: 34,
    includingNegatives: includingNegatives,
    file: file,
    line: line
  )
  // Numeric
  ...
  test(
    mutatingMethod: ({ $0.divideAccordingToEuclid(by: $1) }, "divideAccordingToEuclid"),
    of: 72 as T,
    with: 25,
    resultsIn: 2,
    file: file,
  ...
  if includingNegatives {
    test(
      property: ({ $0.isNatural }, "isNatural"),
      of: 0 as T − 34,
      is: false,
      file: file,
      line: line
  ...
  if includingNegatives {
    test(
      property: ({ $0.isIntegral }, "isIntegral"),
      of: 0 as T − 83,
      is: true,
      file: file,
      line: line
  ...
  test(
    mutatingMethod: ({ $0.round($1) }, "round"),
    of: 26 as T,
    with: .toNearestOrAwayFromZero,
    resultsIn: 26,
    file: file,
    line: line
  )
  ...
  test(
    ...
    of: 52 as T,
    with: (.toNearestOrAwayFromZero, 39),
    resultsIn: 39,
    file: file,
    line: line
  )
  test(
    method: (T.rounded, "rounded"),
    of: 99 as T,
    with: (.toNearestOrAwayFromZero, 10),
    returns: 100,
    file: file,
    line: line
  )
