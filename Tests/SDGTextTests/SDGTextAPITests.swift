...
class SDGTextAPITests : TestCase {
    ...
    func testString() {
        ...
        #if false
        // #workaround(macOS does not fail UTF‐16 on invalid surrogate use, so this is mistaken for UTF‐16. (Swift 4.1.2))
    ...
    func testUnicodeScalar() {
        ...
        func verifyVisible(_ codePoint: Int) {
            if let scalar = UnicodeScalar(codePoint) {
                #if !os(Linux)
                // #workaround(A number of obscure compatibility characters end up empty on Linux. (Swift 4.1.2))
